...
namespace Events_Web_Application.src.Infastructure.Persistence.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        {
            return await appDbContext.Users.Where(x => x.Participations.Any(e => e.EventId == EventId)).ToListAsync();
        }

        public async Task<ICollection<Changes>> GetChanges()
        {
            return await appDbContext.Changes.ToListAsync();
        }

        public async Task<User> GetByUserIdAsync(int UserId)
        {
            return await appDbContext.Users.Where(x => x.Id == UserId).FirstOrDefaultAsync();
        }
