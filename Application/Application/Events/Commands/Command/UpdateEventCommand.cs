...
namespace Events_Web_Application.src.Application.Events.Commands.Command
{
    public class UpdateEventCommand : IRequest<int>
    {
        ...
        public UpdateEventCommand(int id, string name, string description, DateTime date, string place, Category category, int maxUser, ICollection<string> photoPath, ICollection<Participation> participations)
        {
            Id = id;
            Name = name;
